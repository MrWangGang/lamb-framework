package com.lamb.lambframework.core.aspect.config;

/**
 * Created by WangGang on 2017/7/5 0005.
 * E-mail userbean@outlook.com
 * The final interpretation of this procedure is owned by the author
 */
public class JsonSymbolicFinalConfig {


    public  static final String   DEFAULT_SUCCESS_SERVICE_CODE = "00000";

    public  static final String   DEFAULT_SUCCESS_BUSINESS_CODE = "10000";

    public  static final String   DEFAULT_SUCCESS_SERVICE_MESSAGE = "接口调用成功";

    public  static final String   DEFAULT_SUCCESS_BUSINESS_MESSAGE = "业务受理成功";

}
